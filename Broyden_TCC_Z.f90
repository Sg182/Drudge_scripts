...
Subroutine BroydenIterTCC_Z(Z1,Z2,T1,T2,BCSU,BCSV,NAO,pBrdIn,DoCCDIn, &
...
! Set things up 
 ...
 ! y contains T1 T2
   ...
   ! Write(*,*) "After SetUp: NAOBrd = ", NAOBrd, " nBrd = ", nBrd
    ...
    Call SetUpBroyden_Z(BCSU,BCSV,NAO,T1,T2, &
...
Subroutine SetUpBroyden_Z(BCSU,BCSV,NAO,T1,T2, &
                            H00,H20,H11,H02,H40,H31,H22,HT22,H13,H04)
    Implicit None
...
! Allocate and continue copying variables
    ...
    Allocate(T1Brd(NAO), T2Brd(NAO,NAO), Stat=IAlloc)
    If(IAlloc /= 0) Stop "Could not allocate in SetUpBroyden_Z (T1LamBrd/T2LamBrd)"
