...
# --- split at top-level + / - only (outside {}, (), []) ---
terms = []
brace = paren = brack = 0
curr = []
i = 0
while i < len(raw):
    ...
    # split BEFORE a top-level + or - (binary operators between terms)
    if brace == paren == brack == 0 and ch in '+-':
        # finish previous term (if any)
        if curr:
            terms.append(''.join(curr).strip())
            curr = []
        # start new term with the operator
        # include trailing spaces after operator
        j = i + 1
...
# Write TeX: each top-level term on its own aligned line.
# Use \[ ... \] and \resizebox so the whole block fits the line width.
out = "bcs_Normal-ordered_3_body.tex"
with open(out, "w") as f:
    f.write(r"\documentclass{article}" + "\n")
    f.write(r"\usepackage[a4paper,margin=1.75cm]{geometry}" + "\n")
    f.write(r"\usepackage{amsmath,amssymb,mathtools,graphicx}" + "\n")
    f.write(r"\begin{document}" + "\n\n")
    f.write(r"\[" + "\n")
    f.write(r"\resizebox{\linewidth}{!}{$" + "\n")
    f.write(r"\begin{aligned}" + "\n")
    for k, term in enumerate(terms):
        # first line has no leading operator; later lines already begin with + or -
        line = term if k == 0 else term
        f.write("  & " + line + (r" \\" if k < len(terms) - 1 else "") + "\n")
...
with dr.report('CCSD_3_body.html','BCS CCSD') as rep:
    rep.add('SFS Hamiltonian',expr)
