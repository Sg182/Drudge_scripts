    
    Subroutine SFS(u,v,H010,H001,H100,H020,H110,H011,H120,H021,NAO, &
               W000,W001,W100,W010,W101,W200,W002,W110,W011,W020, &
               W030,W111,W120,W210,W021,W012,W201,W102,W003,W300)
    
    Use Precision
    Implicit None
    Integer                        :: NAO
    Complex(Kind=pr), Intent(in)   :: u(NAO), v(NAO)
    Complex(Kind=pr), Intent(in)   :: H010(NAO),H001(NAO), H100(NAO)
    Complex(Kind=pr), Intent(in)   :: H020(NAO,NAO), H110(NAO,NAO), H011(NAO,NAO)
    Complex(Kind=pr), Intent(in)   :: H120(NAO,NAO,NAO),H021(NAO,NAO,NAO)
    Complex(Kind=pr), Intent(out)  :: W000
    Complex(Kind=pr), Intent(out)  :: W001(NAO), W100(NAO), W010(NAO)
    Complex(Kind=pr), Intent(out)  :: W101(NAO,NAO), W200(NAO,NAO), W002(NAO,NAO)
    Complex(Kind=pr), Intent(out)  :: W110(NAO,NAO), W011(NAO,NAO),W020(NAO,NAO)
    Complex(Kind=pr), Intent(out)  :: W030(NAO,NAO,NAO), W111(NAO,NAO,NAO), W120(NAO,NAO,NAO)
    Complex(Kind=pr), Intent(out)  :: W210(NAO,NAO,NAO), W021(NAO,NAO,NAO), W012(NAO,NAO,NAO)
    Complex(Kind=pr), Intent(out)  :: W003(NAO,NAO,NAO), W300(NAO,NAO,NAO)
    Complex(Kind=pr), Intent(out)  :: W201(NAO,NAO,NAO), W102(NAO,NAO,NAO)
    complex(kind=pr)             :: avg,tmp
    
    complex(kind=pr) , dimension(:, :, :), allocatable :: tau0

    complex(kind=pr) , dimension(:), allocatable :: tau1

    complex(kind=pr) , dimension(:, :), allocatable :: tau2

    complex(kind=pr) , dimension(:, :), allocatable :: tau3

    complex(kind=pr) , dimension(:), allocatable :: tau4

    complex(kind=pr) , dimension(:), allocatable :: tau5

    complex(kind=pr) , dimension(:, :), allocatable :: tau6

    complex(kind=pr) , dimension(:), allocatable :: tau7

    complex(kind=pr) , dimension(:), allocatable :: tau8

    complex(kind=pr) , dimension(:), allocatable :: tau9

    complex(kind=pr) , dimension(:), allocatable :: tau10

    complex(kind=pr) , dimension(:), allocatable :: tau11

    complex(kind=pr) , dimension(:), allocatable :: tau12

    complex(kind=pr) , dimension(:), allocatable :: tau13

    complex(kind=pr) , dimension(:, :), allocatable :: tau14

    complex(kind=pr) , dimension(:), allocatable :: tau15

    complex(kind=pr) , dimension(:), allocatable :: tau16

    complex(kind=pr) , dimension(:), allocatable :: tau17

    complex(kind=pr) , dimension(:), allocatable :: tau18

    complex(kind=pr) , dimension(:), allocatable :: tau19

    complex(kind=pr) , dimension(:), allocatable :: tau20

    complex(kind=pr) , dimension(:, :), allocatable :: tau21

    complex(kind=pr) , dimension(:), allocatable :: tau22

    complex(kind=pr) , dimension(:, :), allocatable :: tau23

    complex(kind=pr) , dimension(:, :), allocatable :: tau24

    complex(kind=pr) , dimension(:), allocatable :: tau25

    complex(kind=pr) , dimension(:), allocatable :: tau26

    complex(kind=pr) , dimension(:, :), allocatable :: tau27

    complex(kind=pr) , dimension(:, :), allocatable :: tau28

    complex(kind=pr) , dimension(:), allocatable :: tau29

    complex(kind=pr) , dimension(:), allocatable :: tau30

    complex(kind=pr) , dimension(:), allocatable :: tau31

    complex(kind=pr) , dimension(:, :), allocatable :: tau32

    complex(kind=pr) , dimension(:), allocatable :: tau33

    complex(kind=pr) , dimension(:), allocatable :: tau34

    complex(kind=pr) , dimension(:), allocatable :: tau35

    complex(kind=pr) , dimension(:), allocatable :: tau36

    complex(kind=pr) , dimension(:), allocatable :: tau37

    complex(kind=pr) , dimension(:, :), allocatable :: tau38

    complex(kind=pr) , dimension(:, :), allocatable :: tau39

    complex(kind=pr) , dimension(:, :), allocatable :: tau40

    complex(kind=pr) , dimension(:, :), allocatable :: tau41

    complex(kind=pr) , dimension(:, :), allocatable :: tau42

    complex(kind=pr) , dimension(:, :), allocatable :: tau43

    complex(kind=pr) , dimension(:, :), allocatable :: tau44

    complex(kind=pr) , dimension(:, :), allocatable :: tau45

    complex(kind=pr) , dimension(:, :), allocatable :: tau46

    complex(kind=pr) , dimension(:, :), allocatable :: tau47

    complex(kind=pr) , dimension(:, :), allocatable :: tau48

    complex(kind=pr) , dimension(:, :), allocatable :: tau49

    complex(kind=pr) , dimension(:, :), allocatable :: tau50

    complex(kind=pr) , dimension(:, :), allocatable :: tau51

    complex(kind=pr) , dimension(:, :), allocatable :: tau52

    complex(kind=pr) , dimension(:, :), allocatable :: tau53

    complex(kind=pr) , dimension(:, :), allocatable :: tau54

    complex(kind=pr) , dimension(:, :), allocatable :: tau55

    complex(kind=pr) , dimension(:, :), allocatable :: tau56

    complex(kind=pr) , dimension(:, :), allocatable :: tau57

    complex(kind=pr) , dimension(:, :), allocatable :: tau58

    complex(kind=pr) , dimension(:), allocatable :: tau59

    complex(kind=pr) , dimension(:, :), allocatable :: tau60

    complex(kind=pr) , dimension(:, :), allocatable :: tau61

    complex(kind=pr) , dimension(:, :), allocatable :: tau62

    complex(kind=pr) , dimension(:), allocatable :: tau63

    complex(kind=pr) , dimension(:, :), allocatable :: tau64

    complex(kind=pr) , dimension(:, :), allocatable :: tau65

    complex(kind=pr) , dimension(:), allocatable :: tau66

    complex(kind=pr) , dimension(:, :), allocatable :: tau67

    complex(kind=pr) , dimension(:, :, :), allocatable :: tau68

    complex(kind=pr) , dimension(:, :, :), allocatable :: tau69

    complex(kind=pr) , dimension(:, :), allocatable :: tau70

    complex(kind=pr) , dimension(:, :, :), allocatable :: tau71

    complex(kind=pr) , dimension(:, :, :), allocatable :: tau72


    allocate(tau0(NAO, NAO, NAO))
    tau0 = 0.0

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                tau0(p, q, r) = tau0(p, q, r) + ( &
                    h021(q, p, r)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                tau0(p, q, r) = tau0(p, q, r) + ( &
                    h120(r, q, p)&
                )
    
            end do
        end do
    end do

    allocate(tau1(NAO))
    tau1 = 0.0

    do p=1, NAO
        do r=1, NAO
            do q=1, NAO
                tau1(p) = tau1(p) + ( &
                    v(q)**2 * v(r)**2 * tau0(q, r, p)&
                )
            end do
        end do
    end do

    allocate(tau5(NAO))
    tau5 = 0.0

    do p=1, NAO
    
        tau5(p) = tau5(p) + ( &
            4 * tau1(p)&
        )
    
    end do

    deallocate(tau1)

    allocate(tau13(NAO))
    tau13 = 0.0

    do p=1, NAO
        do r=1, NAO
            do q=1, NAO
                tau13(p) = tau13(p) + ( &
                    v(r)**2 * u(q) * v(q) * tau0(r, p, q)&
                )
            end do
        end do
    end do

    allocate(tau17(NAO))
    tau17 = 0.0

    do p=1, NAO
    
        tau17(p) = tau17(p) + ( &
            4 * tau13(p)&
        )
    
    end do

    allocate(tau37(NAO))
    tau37 = 0.0

    do p=1, NAO
    
        tau37(p) = tau37(p) + ( &
            4 * tau13(p)&
        )
    
    end do

    deallocate(tau13)

    allocate(tau42(NAO, NAO))
    tau42 = 0.0

    do q=1, NAO
        do p=1, NAO
            do r=1, NAO
                tau42(p, q) = tau42(p, q) + ( &
                    u(r) * v(r) * tau0(p, q, r)&
                )
            end do
        end do
    end do

    allocate(tau43(NAO, NAO))
    tau43 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau43(p, q) = tau43(p, q) + ( &
                tau42(q, p)&
            )
    
        end do
    end do

    deallocate(tau42)

    allocate(tau55(NAO, NAO))
    tau55 = 0.0

    do q=1, NAO
        do p=1, NAO
            do r=1, NAO
                tau55(p, q) = tau55(p, q) + ( &
                    v(r)**2 * tau0(r, p, q)&
                )
            end do
        end do
    end do

    allocate(tau56(NAO, NAO))
    tau56 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau56(p, q) = tau56(p, q) + ( &
                4 * tau55(p, q)&
            )
    
        end do
    end do

    deallocate(tau55)

    W012 = 0.0

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W012(p, q, r) = W012(p, q, r) - ( &
                    4 * u(p) * u(q) * u(r) * v(p) * v(q) * v(r) * tau0(r, q, p)&
                )
    
            end do
        end do
    end do

    W210 = 0.0

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W210(p, q, r) = W210(p, q, r) - ( &
                    4 * u(p) * u(q) * u(r) * v(p) * v(q) * v(r) * tau0(q, p, r)&
                )
    
            end do
        end do
    end do

    W111 = 0.0

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W111(p, q, r) = W111(p, q, r) - ( &
                    8 * u(p) * u(q) * u(r) * v(p) * v(q) * v(r) * tau0(r, p, q)&
                )
    
            end do
        end do
    end do

    deallocate(tau0)

    allocate(tau2(NAO, NAO))
    tau2 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau2(p, q) = tau2(p, q) + ( &
                h021(p, p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau2(p, q) = tau2(p, q) + ( &
                h120(q, p, p)&
            )
    
        end do
    end do

    allocate(tau3(NAO, NAO))
    tau3 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau3(p, q) = tau3(p, q) + ( &
                2 * u(p)**2 * tau2(p, q)&
            )
    
        end do
    end do

    allocate(tau18(NAO))
    tau18 = 0.0

    do p=1, NAO
        do q=1, NAO
            tau18(p) = tau18(p) + ( &
                u(q) * v(q) * tau2(p, q)&
            )
        end do
    end do

    allocate(tau19(NAO))
    tau19 = 0.0

    do p=1, NAO
    
        tau19(p) = tau19(p) + ( &
            tau18(p)&
        )
    
    end do

    deallocate(tau18)

    allocate(tau57(NAO, NAO))
    tau57 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau57(p, q) = tau57(p, q) - ( &
                2 * v(p)**3 * tau2(p, q)&
            )
    
        end do
    end do

    W110 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            W110(p, q) = W110(p, q) - ( &
                4 * u(p)**3 * u(q) * v(p) * v(q) * tau2(p, q)&
            )
    
        end do
    end do

    W011 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            W011(p, q) = W011(p, q) - ( &
                4 * u(q)**3 * u(p) * v(p) * v(q) * tau2(q, p)&
            )
    
        end do
    end do

    deallocate(tau2)

    do q=1, NAO
        do p=1, NAO
    
            tau3(p, q) = tau3(p, q) + ( &
                h011(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau3(p, q) = tau3(p, q) + ( &
                h110(q, p)&
            )
    
        end do
    end do

    allocate(tau4(NAO))
    tau4 = 0.0

    do p=1, NAO
        do q=1, NAO
            tau4(p) = tau4(p) + ( &
                v(q)**2 * tau3(q, p)&
            )
        end do
    end do

    deallocate(tau3)

    do p=1, NAO
    
        tau5(p) = tau5(p) + ( &
            2 * tau4(p)&
        )
    
    end do

    deallocate(tau4)

    do p=1, NAO
    
        tau5(p) = tau5(p) + ( &
            h001(p)&
        )
    
    end do

    do p=1, NAO
    
        tau5(p) = tau5(p) + ( &
            h100(p)&
        )
    
    end do

    allocate(tau10(NAO))
    tau10 = 0.0

    do p=1, NAO
    
        tau10(p) = tau10(p) + ( &
            tau5(p) * v(p)&
        )
    
    end do

    deallocate(tau5)

    allocate(tau6(NAO, NAO))
    tau6 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau6(p, q) = tau6(p, q) + ( &
                h021(p, q, p)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau6(p, q) = tau6(p, q) + ( &
                h120(p, p, q)&
            )
    
        end do
    end do

    allocate(tau7(NAO))
    tau7 = 0.0

    do p=1, NAO
        do q=1, NAO
            tau7(p) = tau7(p) + ( &
                v(q)**2 * tau6(p, q)&
            )
        end do
    end do

    allocate(tau8(NAO))
    tau8 = 0.0

    do p=1, NAO
    
        tau8(p) = tau8(p) + ( &
            4 * tau7(p)&
        )
    
    end do

    deallocate(tau7)

    allocate(tau16(NAO))
    tau16 = 0.0

    do p=1, NAO
        do q=1, NAO
            tau16(p) = tau16(p) + ( &
                v(q)**3 * u(q) * tau6(q, p)&
            )
        end do
    end do

    do p=1, NAO
    
        tau17(p) = tau17(p) - ( &
            4 * tau16(p)&
        )
    
    end do

    do p=1, NAO
    
        tau37(p) = tau37(p) - ( &
            4 * tau16(p)&
        )
    
    end do

    deallocate(tau16)

    allocate(tau58(NAO, NAO))
    tau58 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau58(p, q) = tau58(p, q) + ( &
                4 * v(q)**3 * v(p) * tau6(q, p)&
            )
    
        end do
    end do

    allocate(tau62(NAO, NAO))
    tau62 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau62(p, q) = tau62(p, q) + ( &
                4 * v(p)**3 * tau6(p, q)&
            )
    
        end do
    end do

    deallocate(tau6)

    do p=1, NAO
    
        tau8(p) = tau8(p) + ( &
            h011(p, p)&
        )
    
    end do

    do p=1, NAO
    
        tau8(p) = tau8(p) + ( &
            h110(p, p)&
        )
    
    end do

    do p=1, NAO
    
        tau10(p) = tau10(p) - ( &
            2 * v(p)**3 * tau8(p)&
        )
    
    end do

    deallocate(tau8)

    allocate(tau9(NAO))
    tau9 = 0.0

    do p=1, NAO
    
        tau9(p) = tau9(p) + ( &
            h021(p, p, p)&
        )
    
    end do

    do p=1, NAO
    
        tau9(p) = tau9(p) + ( &
            h120(p, p, p)&
        )
    
    end do

    do p=1, NAO
    
        tau10(p) = tau10(p) + ( &
            4 * v(p)**5 * tau9(p)&
        )
    
    end do

    W000 = 0.0

    
    do p=1, NAO
        W000 = W000 + ( &
            tau10(p) * u(p)&
        )
    end do
    

    W010 = 0.0

    do p=1, NAO
    
        W010(p) = W010(p) - ( &
            tau10(p) * u(p)&
        )
    
    end do

    deallocate(tau10)

    
    do p=1, NAO
        W000 = W000 - ( &
            4 * u(p)**3 * v(p)**3 * tau9(p)&
        )
    end do
    

    do p=1, NAO
    
        W010(p) = W010(p) + ( &
            4 * u(p)**3 * v(p)**3 * tau9(p)&
        )
    
    end do

    deallocate(tau9)

    allocate(tau11(NAO))
    tau11 = 0.0

    do p=1, NAO
        do q=1, NAO
            tau11(p) = tau11(p) + ( &
                v(q)**2 * h020(p, q)&
            )
        end do
    end do

    allocate(tau12(NAO))
    tau12 = 0.0

    do p=1, NAO
    
        tau12(p) = tau12(p) + ( &
            2 * tau11(p)&
        )
    
    end do

    do p=1, NAO
    
        tau17(p) = tau17(p) + ( &
            4 * tau11(p)&
        )
    
    end do

    do p=1, NAO
    
        tau37(p) = tau37(p) + ( &
            4 * tau11(p)&
        )
    
    end do

    deallocate(tau11)

    do p=1, NAO
    
        tau12(p) = tau12(p) + ( &
            h010(p)&
        )
    
    end do

    do p=1, NAO
    
        tau12(p) = tau12(p) + ( &
            2 * u(p)**2 * h020(p, p)&
        )
    
    end do

    
    do p=1, NAO
        W000 = W000 + ( &
            2 * v(p)**2 * tau12(p)&
        )
    end do
    

    deallocate(tau12)

    allocate(tau14(NAO, NAO))
    tau14 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau14(p, q) = tau14(p, q) + ( &
                h011(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau14(p, q) = tau14(p, q) + ( &
                h110(q, p)&
            )
    
        end do
    end do

    allocate(tau15(NAO))
    tau15 = 0.0

    do p=1, NAO
        do q=1, NAO
            tau15(p) = tau15(p) + ( &
                u(q) * v(q) * tau14(p, q)&
            )
        end do
    end do

    deallocate(tau14)

    do p=1, NAO
    
        tau17(p) = tau17(p) + ( &
            tau15(p)&
        )
    
    end do

    do p=1, NAO
    
        tau37(p) = tau37(p) + ( &
            tau15(p)&
        )
    
    end do

    deallocate(tau15)

    do p=1, NAO
    
        tau17(p) = tau17(p) + ( &
            h010(p)&
        )
    
    end do

    allocate(tau20(NAO))
    tau20 = 0.0

    do p=1, NAO
    
        tau20(p) = tau20(p) + ( &
            tau17(p) * v(p)&
        )
    
    end do

    do p=1, NAO
    
        W010(p) = W010(p) + ( &
            u(p)**2 * tau17(p)&
        )
    
    end do

    deallocate(tau17)

    do p=1, NAO
    
        tau19(p) = tau19(p) + ( &
            h020(p, p)&
        )
    
    end do

    do p=1, NAO
    
        tau20(p) = tau20(p) - ( &
            2 * v(p)**3 * tau19(p)&
        )
    
    end do

    W100 = 0.0

    do p=1, NAO
    
        W100(p) = W100(p) + ( &
            2 * tau20(p) * u(p)&
        )
    
    end do

    W001 = 0.0

    do p=1, NAO
    
        W001(p) = W001(p) + ( &
            2 * tau20(p) * u(p)&
        )
    
    end do

    deallocate(tau20)

    do p=1, NAO
    
        tau37(p) = tau37(p) + ( &
            4 * u(p)**2 * tau19(p)&
        )
    
    end do

    do p=1, NAO
    
        W100(p) = W100(p) + ( &
            4 * u(p)**3 * tau19(p) * v(p)&
        )
    
    end do

    do p=1, NAO
    
        W001(p) = W001(p) + ( &
            4 * u(p)**3 * tau19(p) * v(p)&
        )
    
    end do

    deallocate(tau19)

    allocate(tau21(NAO, NAO))
    tau21 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau21(p, q) = tau21(p, q) + ( &
                h021(p, q, p)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau21(p, q) = tau21(p, q) + ( &
                2 * h120(p, p, q)&
            )
    
        end do
    end do

    allocate(tau22(NAO))
    tau22 = 0.0

    do p=1, NAO
        do q=1, NAO
            tau22(p) = tau22(p) + ( &
                4 * v(q)**2 * tau21(p, q)&
            )
        end do
    end do

    allocate(tau45(NAO, NAO))
    tau45 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau45(p, q) = tau45(p, q) + ( &
                4 * v(p)**2 * tau21(p, q)&
            )
    
        end do
    end do

    allocate(tau60(NAO, NAO))
    tau60 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau60(p, q) = tau60(p, q) + ( &
                4 * u(q)**2 * tau21(q, p)&
            )
    
        end do
    end do

    deallocate(tau21)

    do p=1, NAO
    
        tau22(p) = tau22(p) + ( &
            h011(p, p)&
        )
    
    end do

    do p=1, NAO
    
        tau22(p) = tau22(p) + ( &
            2 * h110(p, p)&
        )
    
    end do

    allocate(tau26(NAO))
    tau26 = 0.0

    do p=1, NAO
    
        tau26(p) = tau26(p) - ( &
            2 * v(p)**2 * tau22(p)&
        )
    
    end do

    deallocate(tau22)

    allocate(tau23(NAO, NAO))
    tau23 = 0.0

    do q=1, NAO
        do p=1, NAO
            do r=1, NAO
                tau23(p, q) = tau23(p, q) + ( &
                    v(r)**2 * h120(p, q, r)&
                )
            end do
        end do
    end do

    allocate(tau24(NAO, NAO))
    tau24 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau24(p, q) = tau24(p, q) + ( &
                2 * tau23(p, q)&
            )
    
        end do
    end do

    allocate(tau38(NAO, NAO))
    tau38 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau38(p, q) = tau38(p, q) + ( &
                4 * tau23(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau45(p, q) = tau45(p, q) - ( &
                4 * tau23(p, q)&
            )
    
        end do
    end do

    allocate(tau50(NAO, NAO))
    tau50 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau50(p, q) = tau50(p, q) + ( &
                4 * tau23(p, q)&
            )
    
        end do
    end do

    deallocate(tau23)

    do q=1, NAO
        do p=1, NAO
    
            tau24(p, q) = tau24(p, q) + ( &
                h110(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau24(p, q) = tau24(p, q) + ( &
                2 * u(q)**2 * h120(p, q, q)&
            )
    
        end do
    end do

    allocate(tau25(NAO))
    tau25 = 0.0

    do p=1, NAO
        do q=1, NAO
            tau25(p) = tau25(p) + ( &
                v(q)**2 * tau24(p, q)&
            )
        end do
    end do

    deallocate(tau24)

    do p=1, NAO
    
        tau26(p) = tau26(p) + ( &
            2 * tau25(p)&
        )
    
    end do

    allocate(tau35(NAO))
    tau35 = 0.0

    do p=1, NAO
    
        tau35(p) = tau35(p) + ( &
            2 * tau25(p)&
        )
    
    end do

    deallocate(tau25)

    do p=1, NAO
    
        tau26(p) = tau26(p) + ( &
            h100(p)&
        )
    
    end do

    do p=1, NAO
    
        tau26(p) = tau26(p) + ( &
            16 * v(p)**4 * h021(p, p, p)&
        )
    
    end do

    do p=1, NAO
    
        tau26(p) = tau26(p) + ( &
            16 * v(p)**4 * h120(p, p, p)&
        )
    
    end do

    do p=1, NAO
    
        W100(p) = W100(p) + ( &
            u(p)**2 * tau26(p)&
        )
    
    end do

    deallocate(tau26)

    allocate(tau27(NAO, NAO))
    tau27 = 0.0

    do q=1, NAO
        do p=1, NAO
            do r=1, NAO
                tau27(p, q) = tau27(p, q) + ( &
                    v(r)**2 * h021(p, r, q)&
                )
            end do
        end do
    end do

    allocate(tau28(NAO, NAO))
    tau28 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau28(p, q) = tau28(p, q) + ( &
                2 * tau27(p, q)&
            )
    
        end do
    end do

    allocate(tau40(NAO, NAO))
    tau40 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau40(p, q) = tau40(p, q) + ( &
                4 * tau27(p, q)&
            )
    
        end do
    end do

    allocate(tau48(NAO, NAO))
    tau48 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau48(p, q) = tau48(p, q) - ( &
                4 * tau27(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau60(p, q) = tau60(p, q) + ( &
                4 * tau27(p, q)&
            )
    
        end do
    end do

    deallocate(tau27)

    do q=1, NAO
        do p=1, NAO
    
            tau28(p, q) = tau28(p, q) + ( &
                h011(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau28(p, q) = tau28(p, q) + ( &
                2 * u(p)**2 * h021(p, p, q)&
            )
    
        end do
    end do

    allocate(tau29(NAO))
    tau29 = 0.0

    do p=1, NAO
        do q=1, NAO
            tau29(p) = tau29(p) + ( &
                v(q)**2 * tau28(q, p)&
            )
        end do
    end do

    deallocate(tau28)

    allocate(tau30(NAO))
    tau30 = 0.0

    do p=1, NAO
    
        tau30(p) = tau30(p) + ( &
            2 * tau29(p)&
        )
    
    end do

    allocate(tau34(NAO))
    tau34 = 0.0

    do p=1, NAO
    
        tau34(p) = tau34(p) + ( &
            2 * tau29(p)&
        )
    
    end do

    deallocate(tau29)

    do p=1, NAO
    
        tau30(p) = tau30(p) + ( &
            h001(p)&
        )
    
    end do

    do p=1, NAO
    
        tau30(p) = tau30(p) + ( &
            4 * u(p)**4 * h021(p, p, p)&
        )
    
    end do

    do p=1, NAO
    
        tau30(p) = tau30(p) + ( &
            8 * u(p)**4 * h120(p, p, p)&
        )
    
    end do

    do p=1, NAO
    
        W100(p) = W100(p) - ( &
            v(p)**2 * tau30(p)&
        )
    
    end do

    deallocate(tau30)

    allocate(tau31(NAO))
    tau31 = 0.0

    do p=1, NAO
    
        tau31(p) = tau31(p) + ( &
            h011(p, p)&
        )
    
    end do

    do p=1, NAO
        do q=1, NAO
            tau31(p) = tau31(p) + ( &
                4 * v(q)**2 * h021(p, q, p)&
            )
        end do
    end do

    do p=1, NAO
    
        W100(p) = W100(p) + ( &
            2 * v(p)**4 * tau31(p)&
        )
    
    end do

    deallocate(tau31)

    allocate(tau32(NAO, NAO))
    tau32 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau32(p, q) = tau32(p, q) + ( &
                h120(p, p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau32(p, q) = tau32(p, q) + ( &
                2 * h021(p, q, p)&
            )
    
        end do
    end do

    allocate(tau33(NAO))
    tau33 = 0.0

    do p=1, NAO
        do q=1, NAO
            tau33(p) = tau33(p) + ( &
                4 * v(q)**2 * tau32(p, q)&
            )
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau38(p, q) = tau38(p, q) + ( &
                4 * u(p)**2 * tau32(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau48(p, q) = tau48(p, q) + ( &
                4 * v(q)**2 * tau32(q, p)&
            )
    
        end do
    end do

    deallocate(tau32)

    do p=1, NAO
    
        tau33(p) = tau33(p) + ( &
            h110(p, p)&
        )
    
    end do

    do p=1, NAO
    
        tau33(p) = tau33(p) + ( &
            2 * h011(p, p)&
        )
    
    end do

    do p=1, NAO
    
        tau34(p) = tau34(p) - ( &
            2 * v(p)**2 * tau33(p)&
        )
    
    end do

    deallocate(tau33)

    do p=1, NAO
    
        tau34(p) = tau34(p) + ( &
            h001(p)&
        )
    
    end do

    do p=1, NAO
    
        tau34(p) = tau34(p) + ( &
            16 * v(p)**4 * h021(p, p, p)&
        )
    
    end do

    do p=1, NAO
    
        tau34(p) = tau34(p) + ( &
            16 * v(p)**4 * h120(p, p, p)&
        )
    
    end do

    do p=1, NAO
    
        W001(p) = W001(p) + ( &
            u(p)**2 * tau34(p)&
        )
    
    end do

    deallocate(tau34)

    do p=1, NAO
    
        tau35(p) = tau35(p) + ( &
            h100(p)&
        )
    
    end do

    do p=1, NAO
    
        tau35(p) = tau35(p) + ( &
            8 * u(p)**4 * h021(p, p, p)&
        )
    
    end do

    do p=1, NAO
    
        tau35(p) = tau35(p) + ( &
            4 * u(p)**4 * h120(p, p, p)&
        )
    
    end do

    do p=1, NAO
    
        W001(p) = W001(p) - ( &
            v(p)**2 * tau35(p)&
        )
    
    end do

    deallocate(tau35)

    allocate(tau36(NAO))
    tau36 = 0.0

    do p=1, NAO
    
        tau36(p) = tau36(p) + ( &
            h110(p, p)&
        )
    
    end do

    do p=1, NAO
        do q=1, NAO
            tau36(p) = tau36(p) + ( &
                4 * v(q)**2 * h120(p, p, q)&
            )
        end do
    end do

    do p=1, NAO
    
        W001(p) = W001(p) + ( &
            2 * v(p)**4 * tau36(p)&
        )
    
    end do

    deallocate(tau36)

    do p=1, NAO
    
        tau37(p) = tau37(p) + ( &
            h010(p)&
        )
    
    end do

    do p=1, NAO
    
        W010(p) = W010(p) - ( &
            v(p)**2 * tau37(p)&
        )
    
    end do

    deallocate(tau37)

    do q=1, NAO
        do p=1, NAO
    
            tau38(p, q) = tau38(p, q) + ( &
                h110(p, q)&
            )
    
        end do
    end do

    allocate(tau39(NAO, NAO))
    tau39 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau39(p, q) = tau39(p, q) + ( &
                v(q) * tau38(p, q)&
            )
    
        end do
    end do

    deallocate(tau38)

    do q=1, NAO
        do p=1, NAO
    
            tau39(p, q) = tau39(p, q) - ( &
                2 * v(q)**3 * h120(p, q, q)&
            )
    
        end do
    end do

    W002 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            W002(p, q) = W002(p, q) - ( &
                2 * v(p)**2 * u(q) * tau39(p, q)&
            )
    
        end do
    end do

    deallocate(tau39)

    do q=1, NAO
        do p=1, NAO
    
            tau40(p, q) = tau40(p, q) + ( &
                h011(p, q)&
            )
    
        end do
    end do

    allocate(tau41(NAO, NAO))
    tau41 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau41(p, q) = tau41(p, q) + ( &
                u(p) * tau40(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau41(p, q) = tau41(p, q) + ( &
                2 * u(p)**3 * h021(p, p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W002(p, q) = W002(p, q) + ( &
                2 * u(q)**2 * v(p) * tau41(p, q)&
            )
    
        end do
    end do

    W200 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            W200(p, q) = W200(p, q) - ( &
                2 * v(q)**2 * v(p) * tau41(p, q)&
            )
    
        end do
    end do

    W101 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            W101(p, q) = W101(p, q) - ( &
                2 * v(p)**2 * v(q) * tau41(q, p)&
            )
    
        end do
    end do

    deallocate(tau41)

    do q=1, NAO
        do p=1, NAO
    
            tau43(p, q) = tau43(p, q) + ( &
                h020(q, p)&
            )
    
        end do
    end do

    allocate(tau65(NAO, NAO))
    tau65 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau65(p, q) = tau65(p, q) + ( &
                v(p)**2 * tau43(q, p)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W002(p, q) = W002(p, q) + ( &
                4 * u(p) * u(q) * v(p) * v(q) * tau43(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W200(p, q) = W200(p, q) + ( &
                4 * u(p) * u(q) * v(p) * v(q) * tau43(q, p)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W101(p, q) = W101(p, q) + ( &
                8 * u(p) * u(q) * v(p) * v(q) * tau43(q, p)&
            )
    
        end do
    end do

    allocate(tau44(NAO, NAO))
    tau44 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau44(p, q) = tau44(p, q) + ( &
                2 * v(p)**4 * u(q) * h120(p, p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau44(p, q) = tau44(p, q) - ( &
                u(q)**3 * v(p)**2 * h120(p, q, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W002(p, q) = W002(p, q) + ( &
                4 * v(q) * tau44(p, q)&
            )
    
        end do
    end do

    deallocate(tau44)

    do q=1, NAO
        do p=1, NAO
    
            tau45(p, q) = tau45(p, q) - ( &
                h110(p, q)&
            )
    
        end do
    end do

    allocate(tau46(NAO, NAO))
    tau46 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau46(p, q) = tau46(p, q) + ( &
                v(q) * tau45(p, q)&
            )
    
        end do
    end do

    deallocate(tau45)

    do q=1, NAO
        do p=1, NAO
    
            tau46(p, q) = tau46(p, q) + ( &
                2 * v(q)**3 * h120(p, q, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W200(p, q) = W200(p, q) - ( &
                2 * u(p)**2 * u(q) * tau46(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W101(p, q) = W101(p, q) - ( &
                2 * u(p)**2 * u(q) * tau46(p, q)&
            )
    
        end do
    end do

    deallocate(tau46)

    allocate(tau47(NAO, NAO))
    tau47 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau47(p, q) = tau47(p, q) + ( &
                2 * v(p)**4 * u(q) * h021(p, q, p)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau47(p, q) = tau47(p, q) + ( &
                u(p)**2 * u(q)**3 * h120(p, q, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W200(p, q) = W200(p, q) + ( &
                4 * v(q) * tau47(p, q)&
            )
    
        end do
    end do

    deallocate(tau47)

    do q=1, NAO
        do p=1, NAO
    
            tau48(p, q) = tau48(p, q) - ( &
                h011(p, q)&
            )
    
        end do
    end do

    allocate(tau49(NAO, NAO))
    tau49 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau49(p, q) = tau49(p, q) + ( &
                v(p) * tau48(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau49(p, q) = tau49(p, q) + ( &
                2 * v(p)**3 * h021(p, p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W101(p, q) = W101(p, q) - ( &
                2 * u(q)**2 * u(p) * tau49(p, q)&
            )
    
        end do
    end do

    deallocate(tau49)

    do q=1, NAO
        do p=1, NAO
    
            tau50(p, q) = tau50(p, q) + ( &
                h110(p, q)&
            )
    
        end do
    end do

    allocate(tau51(NAO, NAO))
    tau51 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau51(p, q) = tau51(p, q) + ( &
                v(q) * tau50(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau62(p, q) = tau62(p, q) - ( &
                v(p) * tau50(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau51(p, q) = tau51(p, q) - ( &
                2 * v(q)**3 * h120(p, q, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W101(p, q) = W101(p, q) - ( &
                2 * v(q)**2 * u(p) * tau51(q, p)&
            )
    
        end do
    end do

    deallocate(tau51)

    allocate(tau52(NAO, NAO))
    tau52 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau52(p, q) = tau52(p, q) + ( &
                u(q)**2 * h021(p, p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau52(p, q) = tau52(p, q) - ( &
                v(q)**2 * h120(q, p, p)&
            )
    
        end do
    end do

    allocate(tau53(NAO, NAO))
    tau53 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau53(p, q) = tau53(p, q) + ( &
                u(q)**3 * tau52(q, p)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W011(p, q) = W011(p, q) - ( &
                4 * u(p)**2 * v(p)**2 * tau52(p, q)&
            )
    
        end do
    end do

    deallocate(tau52)

    do q=1, NAO
        do p=1, NAO
    
            tau53(p, q) = tau53(p, q) + ( &
                2 * v(p)**4 * u(q) * h120(p, p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W101(p, q) = W101(p, q) + ( &
                4 * v(p) * tau53(q, p)&
            )
    
        end do
    end do

    deallocate(tau53)

    allocate(tau54(NAO, NAO))
    tau54 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau54(p, q) = tau54(p, q) + ( &
                2 * v(p)**4 * v(q) * h021(p, q, p)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau54(p, q) = tau54(p, q) + ( &
                v(p)**2 * v(q)**3 * h021(q, q, p)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W101(p, q) = W101(p, q) + ( &
                4 * u(q) * tau54(p, q)&
            )
    
        end do
    end do

    deallocate(tau54)

    do q=1, NAO
        do p=1, NAO
    
            tau56(p, q) = tau56(p, q) + ( &
                h011(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau56(p, q) = tau56(p, q) + ( &
                h110(q, p)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau57(p, q) = tau57(p, q) + ( &
                v(p) * tau56(p, q)&
            )
    
        end do
    end do

    deallocate(tau56)

    do q=1, NAO
        do p=1, NAO
    
            tau58(p, q) = tau58(p, q) - ( &
                v(q) * tau57(p, q)&
            )
    
        end do
    end do

    deallocate(tau57)

    do q=1, NAO
        do p=1, NAO
    
            W110(p, q) = W110(p, q) + ( &
                2 * u(p) * u(q) * tau58(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W011(p, q) = W011(p, q) + ( &
                2 * u(p) * u(q) * tau58(q, p)&
            )
    
        end do
    end do

    deallocate(tau58)

    allocate(tau59(NAO))
    tau59 = 0.0

    do p=1, NAO
    
        tau59(p) = tau59(p) + ( &
            u(p)**2&
        )
    
    end do

    do p=1, NAO
    
        tau59(p) = tau59(p) - ( &
            v(p)**2&
        )
    
    end do

    allocate(tau61(NAO, NAO))
    tau61 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau61(p, q) = tau61(p, q) - ( &
                tau59(q) * tau50(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W110(p, q) = W110(p, q) + ( &
                4 * tau59(q) * u(p) * v(p) * tau43(q, p)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W110(p, q) = W110(p, q) + ( &
                4 * v(p)**4 * tau59(q) * h021(p, q, p)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W011(p, q) = W011(p, q) - ( &
                u(q)**2 * tau59(p) * tau48(p, q)&
            )
    
        end do
    end do

    deallocate(tau48)

    do q=1, NAO
        do p=1, NAO
    
            W011(p, q) = W011(p, q) + ( &
                4 * tau59(p) * u(q) * v(q) * tau43(q, p)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W011(p, q) = W011(p, q) - ( &
                v(q)**2 * tau59(p) * tau50(q, p)&
            )
    
        end do
    end do

    deallocate(tau50)

    do q=1, NAO
        do p=1, NAO
    
            W011(p, q) = W011(p, q) + ( &
                4 * v(q)**4 * tau59(p) * h120(q, q, p)&
            )
    
        end do
    end do

    W020 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            W020(p, q) = W020(p, q) + ( &
                tau59(q) * u(p) * tau62(p, q)&
            )
    
        end do
    end do

    deallocate(tau62)

    do q=1, NAO
        do p=1, NAO
    
            W020(p, q) = W020(p, q) - ( &
                tau59(p) * u(q) * v(q) * tau40(p, q)&
            )
    
        end do
    end do

    deallocate(tau40)

    W120 = 0.0

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W120(p, q, r) = W120(p, q, r) - ( &
                    4 * tau59(r) * u(p) * u(q) * v(p) * v(q) * h120(q, r, p)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W120(p, q, r) = W120(p, q, r) - ( &
                    4 * tau59(q) * u(p) * u(r) * v(p) * v(r) * h021(q, p, r)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W012(p, q, r) = W012(p, q, r) - ( &
                    4 * v(q)**2 * tau59(p) * u(r) * v(r) * h120(q, r, p)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W012(p, q, r) = W012(p, q, r) + ( &
                    4 * u(r)**2 * tau59(p) * u(q) * v(q) * h021(q, p, r)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W210(p, q, r) = W210(p, q, r) + ( &
                    4 * u(p)**2 * tau59(r) * u(q) * v(q) * h120(p, r, q)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W210(p, q, r) = W210(p, q, r) - ( &
                    4 * v(q)**2 * tau59(r) * u(p) * v(p) * h021(r, p, q)&
                )
    
            end do
        end do
    end do

    W021 = 0.0

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W021(p, q, r) = W021(p, q, r) - ( &
                    4 * tau59(q) * u(p) * u(r) * v(p) * v(r) * h120(p, r, q)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W021(p, q, r) = W021(p, q, r) - ( &
                    4 * tau59(p) * u(q) * u(r) * v(q) * v(r) * h021(r, p, q)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W111(p, q, r) = W111(p, q, r) - ( &
                    4 * v(r)**2 * tau59(q) * u(p) * v(p) * h120(r, q, p)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W111(p, q, r) = W111(p, q, r) + ( &
                    4 * u(p)**2 * tau59(q) * u(r) * v(r) * h120(p, r, q)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W111(p, q, r) = W111(p, q, r) - ( &
                    4 * v(p)**2 * tau59(q) * u(r) * v(r) * h021(r, q, p)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W111(p, q, r) = W111(p, q, r) + ( &
                    4 * u(r)**2 * tau59(q) * u(p) * v(p) * h021(p, q, r)&
                )
    
            end do
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            tau60(p, q) = tau60(p, q) + ( &
                h011(p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W110(p, q) = W110(p, q) - ( &
                v(p)**2 * tau59(q) * tau60(q, p)&
            )
    
        end do
    end do

    deallocate(tau59)

    deallocate(tau60)

    do q=1, NAO
        do p=1, NAO
    
            tau61(p, q) = tau61(p, q) + ( &
                4 * u(q)**2 * v(q)**2 * h120(p, q, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W110(p, q) = W110(p, q) - ( &
                u(p)**2 * tau61(p, q)&
            )
    
        end do
    end do

    deallocate(tau61)

    allocate(tau63(NAO))
    tau63 = 0.0

    do p=1, NAO
    
        tau63(p) = tau63(p) - ( &
            u(p)**2&
        )
    
    end do

    do p=1, NAO
    
        tau63(p) = tau63(p) + ( &
            2 * v(p)**2&
        )
    
    end do

    allocate(tau64(NAO, NAO))
    tau64 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau64(p, q) = tau64(p, q) - ( &
                tau63(p) * tau43(q, p)&
            )
    
        end do
    end do

    deallocate(tau43)

    allocate(tau70(NAO, NAO))
    tau70 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau70(p, q) = tau70(p, q) - ( &
                u(q)**2 * tau63(p)&
            )
    
        end do
    end do

    deallocate(tau63)

    do q=1, NAO
        do p=1, NAO
    
            tau64(p, q) = tau64(p, q) + ( &
                4 * v(q)**2 * u(p) * v(p) * h021(q, q, p)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W020(p, q) = W020(p, q) + ( &
                u(p)**2 * tau64(q, p)&
            )
    
        end do
    end do

    deallocate(tau64)

    do q=1, NAO
        do p=1, NAO
    
            tau65(p, q) = tau65(p, q) + ( &
                4 * u(q)**2 * u(p) * v(p) * h120(p, q, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W020(p, q) = W020(p, q) + ( &
                v(q)**2 * tau65(p, q)&
            )
    
        end do
    end do

    deallocate(tau65)

    allocate(tau66(NAO))
    tau66 = 0.0

    do p=1, NAO
    
        tau66(p) = tau66(p) + ( &
            2 * u(p)**2&
        )
    
    end do

    do p=1, NAO
    
        tau66(p) = tau66(p) - ( &
            v(p)**2&
        )
    
    end do

    allocate(tau67(NAO, NAO))
    tau67 = 0.0

    do q=1, NAO
        do p=1, NAO
    
            tau67(p, q) = tau67(p, q) - ( &
                v(q)**2 * tau66(p)&
            )
    
        end do
    end do

    deallocate(tau66)

    do q=1, NAO
        do p=1, NAO
    
            tau67(p, q) = tau67(p, q) + ( &
                u(p)**2 * u(q)**2&
            )
    
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W120(p, q, r) = W120(p, q, r) + ( &
                    u(p)**2 * tau67(q, r) * h120(p, r, q)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W120(p, q, r) = W120(p, q, r) - ( &
                    v(p)**2 * tau67(q, r) * h021(r, q, p)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W021(p, q, r) = W021(p, q, r) - ( &
                    v(r)**2 * tau67(p, q) * h120(r, q, p)&
                )
    
            end do
        end do
    end do

    W030 = 0.0

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W030(p, q, r) = W030(p, q, r) - ( &
                    u(r) * v(r) * tau67(p, q) * h021(q, p, r)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W030(p, q, r) = W030(p, q, r) - ( &
                    u(p) * v(p) * tau67(q, r) * h120(p, r, q)&
                )
    
            end do
        end do
    end do

    deallocate(tau67)

    allocate(tau68(NAO, NAO, NAO))
    tau68 = 0.0

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                tau68(p, q, r) = tau68(p, q, r) + ( &
                    u(r)**2 * h021(q, p, r)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                tau68(p, q, r) = tau68(p, q, r) - ( &
                    v(r)**2 * h120(r, q, p)&
                )
    
            end do
        end do
    end do

    W201 = 0.0

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W201(p, q, r) = W201(p, q, r) + ( &
                    4 * u(p) * u(q) * v(p) * v(q) * tau68(q, p, r)&
                )
    
            end do
        end do
    end do

    deallocate(tau68)

    allocate(tau69(NAO, NAO, NAO))
    tau69 = 0.0

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                tau69(p, q, r) = tau69(p, q, r) + ( &
                    v(r)**2 * u(p) * v(p) * h021(p, q, r)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                tau69(p, q, r) = tau69(p, q, r) - ( &
                    u(p)**2 * u(r) * v(r) * h120(p, r, q)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W201(p, q, r) = W201(p, q, r) - ( &
                    8 * u(r) * v(r) * tau69(p, r, q)&
                )
    
            end do
        end do
    end do

    W300 = 0.0

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W300(p, q, r) = W300(p, q, r) - ( &
                    4 * u(q) * v(q) * tau69(p, q, r)&
                )
    
            end do
        end do
    end do

    deallocate(tau69)

    do q=1, NAO
        do p=1, NAO
    
            tau70(p, q) = tau70(p, q) + ( &
                v(p)**2 * v(q)**2&
            )
    
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W021(p, q, r) = W021(p, q, r) + ( &
                    u(r)**2 * tau70(q, p) * h021(q, p, r)&
                )
    
            end do
        end do
    end do

    deallocate(tau70)

    allocate(tau71(NAO, NAO, NAO))
    tau71 = 0.0

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                tau71(p, q, r) = tau71(p, q, r) + ( &
                    v(r)**2 * h021(q, p, r)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                tau71(p, q, r) = tau71(p, q, r) - ( &
                    u(r)**2 * h120(r, q, p)&
                )
    
            end do
        end do
    end do

    W102 = 0.0

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W102(p, q, r) = W102(p, q, r) - ( &
                    4 * u(q) * u(r) * v(q) * v(r) * tau71(r, q, p)&
                )
    
            end do
        end do
    end do

    deallocate(tau71)

    allocate(tau72(NAO, NAO, NAO))
    tau72 = 0.0

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                tau72(p, q, r) = tau72(p, q, r) + ( &
                    u(r)**2 * u(p) * v(p) * h021(p, q, r)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                tau72(p, q, r) = tau72(p, q, r) - ( &
                    v(p)**2 * u(r) * v(r) * h120(p, r, q)&
                )
    
            end do
        end do
    end do

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W102(p, q, r) = W102(p, q, r) + ( &
                    8 * u(p) * v(p) * tau72(q, p, r)&
                )
    
            end do
        end do
    end do

    W003 = 0.0

    do r=1, NAO
        do q=1, NAO
            do p=1, NAO
    
                W003(p, q, r) = W003(p, q, r) + ( &
                    4 * u(q) * v(q) * tau72(p, q, r)&
                )
    
            end do
        end do
    end do

    deallocate(tau72)

    do p=1, NAO
    
        W100(p) = W100(p) - ( &
            4 * v(p)**6 * h021(p, p, p)&
        )
    
    end do

    do p=1, NAO
    
        W001(p) = W001(p) - ( &
            4 * v(p)**6 * h120(p, p, p)&
        )
    
    end do

    do q=1, NAO
        do p=1, NAO
    
            W002(p, q) = W002(p, q) - ( &
                4 * u(q)**2 * v(p)**3 * u(p) * h021(p, p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W200(p, q) = W200(p, q) + ( &
                4 * v(p)**3 * v(q)**2 * u(p) * h021(p, p, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W101(p, q) = W101(p, q) + ( &
                4 * u(p)**2 * u(q)**3 * v(q) * h120(p, q, q)&
            )
    
        end do
    end do

    do q=1, NAO
        do p=1, NAO
    
            W110(p, q) = W110(p, q) + ( &
                4 * u(q)**2 * v(p)**2 * v(q)**2 * h021(q, q, p)&
            )
    
        end do
    end do


    !symmetrize W002 = PpPq
    do p = 1, NAO
    do q = p, NAO
       tmp = 0.5_pr * (W002(p,q) + (W002(q,p)))
       !tmp = 0.5_pr * (W002(p,q) + (conjg(W002(q,p)))
       W002(p,q) = tmp
       W002(q,p) = (tmp)
       W002(p,p) = (0.0_pr,0.0_pr)
    end do
    end do

    W200 = W002
    
    !symmetrize W210
    do r = 1, NAO
    do p = 1, NAO
       W210 (p,p,r) = (0.0_pr,0.0_pr)
    do q = p, NAO
      tmp = 0.5_pr * (W210(p,q,r) + W210(q,p,r))            ! real
      ! tmp = 0.5_pr * (H210(p,q,r) + conjg(H210(q,p,r)))   ! complex "Hermitian" in pq
      W210(p,q,r) = tmp
      W210(q,p,r) = tmp    ! or conjg(tmp) for the complex variant above
      !H210 (p,p,r) = 0.0_pr
    end do
    end do
    end do

    ! symmetrize W012

    do p = 1, NAO
    do q = 1, NAO
        W012(p,q,q) = (0.0_pr,0.0_pr)                  ! or (0.0_pr,0.0_pr) if complex
    do r = q+1, NAO
      tmp = 0.5_pr * (W012(p,q,r) + W012(p,r,q))
      W012(p,q,r) = tmp
      W012(p,r,q) = tmp
    end do
    end do
    end do

    !symmetrize W030

    do p = 1, NAO
    do q = p, NAO
    do r = q, NAO
      avg = ( W030(p,q,r) + W030(p,r,q) + W030(q,p,r) &
            + W030(q,r,p) + W030(r,p,q) + W030(r,q,p) ) / 6.0_pr

      W030(p,q,r) = avg
      W030(p,r,q) = avg
      W030(q,p,r) = avg
      W030(q,r,p) = avg
      W030(r,p,q) = avg
      W030(r,q,p) = avg
    end do
    end do
    end do
    
    ! symmetrize W021
    do r = 1, NAO
    do p = 1, NAO
    do q = p, NAO
      tmp = 0.5_pr * (W021(p,q,r) + W021(q,p,r))
      W021(p,q,r) = tmp
      W021(q,p,r) = tmp
    end do
    end do
    end do
    

    ! symmetrize W201
    do r = 1, NAO
    do p = 1, NAO
      W201(p,p,r) = (0.0_pr,0.0_pr)
    do q = p+1, NAO
      tmp = 0.5_pr * (W201(p,q,r) + W201(q,p,r))
      W201(p,q,r) = tmp
      W201(q,p,r) = tmp
    end do
    end do
    end do



    !Symmetrize W021
    do r = 1, NAO
    do p = 1, NAO
    do q = p, NAO
      tmp = 0.5_pr * (W021(p,q,r) + W021(q,p,r))
      W021(p,q,r) = tmp
      W021(q,p,r) = tmp
    end do
    end do
    end do


    
    !symmetrize W102
    do p = 1, NAO
    do q = 1, NAO
      W102(p,q,q) = (0.0_pr,0.0_pr)
    do r = q+1, NAO
      tmp = 0.5_pr * (W102(p,q,r) + W102(p,r,q))
      W102(p,q,r) = tmp
      W102(p,r,q) = tmp
    end do
    end do
    end do


    !symmetrize W300

    do p = 1, NAO
    do q = 1, NAO
    do r = 1, NAO
      if (p==q .or. q==r .or. p==r) then
        W300(p,q,r) = (0.0_pr,0.0_pr)
      end if
    end do
    end do
    end do

    do p = 1, NAO
    do q = p+1, NAO
    do r = q+1, NAO
      avg = ( W300(p,q,r) + W300(p,r,q) + W300(q,p,r) &
            + W300(q,r,p) + W300(r,p,q) + W300(r,q,p) ) / 6.0_pr
      W300(p,q,r) = avg
      W300(p,r,q) = avg
      W300(q,p,r) = avg
      W300(q,r,p) = avg
      W300(r,p,q) = avg
      W300(r,q,p) = avg
    end do
    end do
    end do

    !symmetrize W003

    W003 = W300

    End subroutine
